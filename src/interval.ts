...
export class Interval implements IInterval {
   ...
   */
  compliments(interval: Interval): boolean {
   ...
   */
  public static intervalType(interval: Interval): 'number' | 'bigint' {
    ...
    const minType = typeof interval.min.number as 'number' | 'bigint';
    const maxType = typeof interval.max.number as 'number' | 'bigint';
    if (minType === maxType) {
      return minType;
    }
    // If types differ, check if one is numeric Infinity/-Infinity
    const minIsInfinite = minType === "number" && !isFinite(interval.min.number as number);
    const maxIsInfinite = maxType === "number" && !isFinite(interval.max.number as number);
    ...
    throw new Error(
      `Invalid interval: Both endpoints must be of the same type (number or bigint), unless one is numeric Infinity/-Infinity. Got ${minType} and ${maxType}.`,
    );
  }
   ...
   */
  public static intersects(i1: Interval, i2: Interval): Interval | null {
    ...
    const candidate: IInterval = { a: newMin, b: newMax };
    
    // Check if the result is valid (handles edge case where min === max but both open)
    if (!Interval.validInterval(candidate)) return null;
   ...
   */
  static union(i1: Interval, i2: Interval): Interval {
   ...
   * ) // Returns [1, 10]
   */
  public static mergeIntervals(a: Interval, b: Interval): Interval {
    // Check if they overlap or complement each other (touch with opposite closure)
    const canMerge = a.overlaps(b) || a.compliments(b);
    
    if (!canMerge) {
      throw new Error(
        `Cannot merge disjoint intervals. [${a.toString()}] and [${b.toString()}] do not overlap or touch.`,
      );
    }
   ...
   * @private
   */
  private validateTypeCompatibility(newValue: IntervalNumber, existingValue: IntervalNumber): void {
    const newType = typeof newValue.number;
    const existingType = typeof existingValue.number;
    const newIsInfinite = newType === "number" && !isFinite(newValue.number as number);
    const existingIsInfinite = existingType === "number" && !isFinite(existingValue.number as number);

    if (newType !== existingType && !newIsInfinite && !existingIsInfinite) {
      throw new Error(
        `Invalid interval. Both numbers must be of the same type. Got ${newType} and ${existingType}.`,
      );
    }
  }
   ...
   * @private
   */
  private validateBoundaryEqualityConstraint(
    endpoint1: IntervalNumber,
    endpoint2: IntervalNumber,
  ): void {
  ...
  public static validInterval(interval: IInterval): boolean {
    ...
    const aIsInfinite =
      typeof interval.a.number === "number" && !isFinite(interval.a.number);
    const bIsInfinite =
      typeof interval.b.number === "number" && !isFinite(interval.b.number);
   ...
   */
  public static validIntervalString(interval: string): boolean {
    try {
   ...
   * @private
   */
  private static toIntervalNumber(
    x: IntervalNumber | NumericValue,
    isClosed = true,
  ): IntervalNumber {
   ...
   */
  public static isIntervalNumber(x: any): x is IntervalNumber {
